...
성공 배지는 완료 결과를 확인할 수 있도록 8초 동안 유지하고 X 버튼으로도
즉시 닫을 수 있게 한다. 성공 배지의 표시 수명과 게시 슬롯의 점유 수명은
분리하여 수정 요청이 완료되는 즉시 새 게시 명령을 허용한다. 새 명령이
들어오면 남아 있던 완료 배지는 새 작업 상태로 교체한다. 실패 작업은
재시도 또는 닫기가 필요하므로 기존처럼 슬롯을 계속 점유한다.
...
- 생성·수정 성공 배지는 8초간 표시되며 X로 즉시 닫을 수 있다.
- 수정 성공 배지가 남아 있어도 게시 슬롯은 즉시 해제되고, 새 게시 명령은
  이전 완료 표시를 안전하게 교체한다.
- 전체 Node 단위 테스트와 iOS Expo export가 통과했다.
